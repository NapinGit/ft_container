...
class RBtree
{
    ...
    Node* find_key(const key_type& k, Node *node)
    {
    	...
    	/*Node *find_key(const Key n, Node *node)
		{
			if (!root)
				return NULL;
			Node *temp = node;
    ...
    Node *find_key(const key_type n, Node *node) const
	{
		if (!root)
			return NULL;
		Node *temp = node;
    ...
    RBtree(): root(NULL), _end(NULL),  compare(), _size(0), _allocator(Allocator())
    {
        this->_nodeAlloc = std::allocator<Node>();
        initialize_end();
    }

    RBtree( const RBtree& other ) :root(NULL), _end(NULL),  compare(), _size(0), _allocator(other._allocator)
	{
		this->_nodeAlloc = std::allocator<Node>();
        initialize_end();
        *this = other;
    ...
    {
        Node *retour = find_key(to_insert.first, this->root);
        if (retour == this->_end)
        {
            ...
            this->_size++;
            retour = find_key(to_insert.first, this->root);
    ...
    void insert(const value_type &data)
    {
        ...
        if (find_key(data.first, this->root) == this->_end)
        {
